    ...
    for (int i = 0; i < nPops; i++)
    {
        for (int j = 0; j < nPops; j++)
        {
            ...
            else if (i == j - 1)
            {
                ...
                if (i == 0)
                    mig_prob.at(i).at(j) = m; // probability of migrating if pop is at beginning of line
            }
            else if (i == j + 1)
            {
                mig_prob.at(i).at(j) = m / 2; // probability of migrating to neighboring pop at left
                if (i == nPops - 1)
                    mig_prob.at(i).at(j) = m; // probability of migrating if pop is at end of line
            }
            else
                mig_prob.at(i).at(j) = 0; // probability of migrating to any other pop
        }
    }
...
int main(int argc, const char *argv[])
{
    ...
    infile << "inLABP.pars";                          // changed
    ms_ss << "tests/outLABP_" << seed << ".sites";    // changed
    sstat_ss << "tests/outLABP_" << seed << ".stats"; // changed

    std::vector<std::string> param_vec(argv + 2, argv + argc);

    Parameters params(infile.str().c_str(), param_vec); // changed

    unsigned int nRuns = params.paramData->nRuns;
    unsigned nSites = params.paramData->n_SNPs;

    /// new migration_matrix reading
    std::string filename = "/Users/varundeepakgudhe/Downloads/Ancestry_Project/laBP_varun/src/migration_matrices.json"; // JSON file name
    std::string selected_time = "500";                                                                                  // Change this as needed

    // Call function to read the migration matrix and assign it to mig_prob
    Matrix mig_prob = readMigrationMatrix(filename, selected_time);

    // Print the assigned matrix for verification
    if (!mig_prob.empty())
    {
        std::cout << "Migration Matrix assigned to my_variable at Time " << selected_time << ":\n";
        // for (const auto& row : my_variable) {
        //     for (double val : row) {
        //         std::cout << val << " ";
        //     }
        //     std::cout << "\n";
    ...
    {
        std::cerr << "Error: mig_prob is empty.\n";
    }

    /// END of new_migration matrix and commented next line
    ...
    for (int timer = 0; timer < nRuns; ++timer)
    {
        ...
        for (int k = 0; k < nSites; ++k)
        {
            ...
            {
                pos = 0;
            } // if not doing fixed segregating sites, all sites are bases in a region with the same gene tree
            ...
            if (mutantCount > 0)
            { // this is a variable site, add to summary statistics
                trees.push_back(tmp);
                varPos.push_back(k);
            }
            else
            { // No variation at this site. Keeping track
                ++novar;
            }
