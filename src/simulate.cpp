...
unsigned short World::simulateGeneration(vector<vector<double>> &mig_prob)
{
    ...
    double totalM = 0;
    double totalC = 0;
    double totalR = 0; // homochariotipic recomb is a normal recom
    double totalH = 0; // heterochariotipic recomb. This is the recomb for outside of inversion part.
    double totalD = 0; // double recombination

    for (cluster_t::iterator i = cluster.begin(); i != cluster.end(); ++i)
    {
        Context cxt = i->first;
        unsigned int k = ctxtNcarriers(cxt); // number of samples
        ...
        if (k > 0)
        {
            ...
            double freqI = getFreqI(pop);                                                // inversion freq of that population                                           //
            double clustSize = worldData->freq.at(clustID) * worldData->popSize.at(pop); // its like 0.8x10000=8000. Multiplying freq with pop size
            if (cxt.inversion == 0)                                                      // redundant. It can be just variable = worldData->freq.at(clustID)
                freqI = 1 - freqI;                                                       // redundant.
            ...
            vector<double> mfreq = getSizebyPop(cxt); // here we get a <pop1*freq, pop2*freq>. For ex: for p1_f=0.8, p1'_f=0.2; p2_f=0.7, p2'_f=0.3
            // So, each time we get <8000,7000> ;<2000,3000>; <8000,7000> ;<2000,3000>.
            vector<double> migs;
            ...
            for (int l = 0; l < worldData->nPops; ++l)
            {
                ...
                {
                    double q1q2 = 0;
                    if (mfreq.at(pop) > 0)                        // Its always going to be greater than 0.
                        q1q2 = mfreq.at(l) / mfreq.at(pop);       // Other/currentpop ex: 7000/8000
                    b_mig_total += mig_prob.at(pop).at(l) * q1q2; // total mig prob i.e pop1 with all the remaining. ex: migmatrix[0][1]x(7000/8000)= maybe (0.2)x(0.9)
                    migs.push_back(b_mig_total);                  // why are pushing each addition migration
                }
                else
                    migs.push_back(0);
            }
            migmap.push_back(migs); // for each iteration in the cluster

            mRate.push_back(k * b_mig_total); // rate is number of carriers in context, times probability of migration in population
            totalM += mRate[clustID];         // why total of all contexts

            if (k > 1) // explain this condition and following 3
            {
                unsigned int a = 2;
                if (clustSize == 0)
                {
                    std::cerr << "Error in SimulateGeneration(): Carriers in seemly empty context\n";
                }
            ...
            for (int carrierID = 0; carrierID < k; ++carrierID) // for our simulation, this may not iterate as its(getHeterolength) always going to be 0.
            {
    ...
    std::cout << "Total M :\n"
              << totalM;
    if (worldData->kingman)
    { // simulation is running on Kingman approx
        if (timeCheck(waiting_t, Rate))
        { // waiting time exceeds time to next epoch, need to update freqs/sizes and skip the events
            updateToNextEpoch();
        }
        else
        { // waiting time looks good, run Kingman events
            worldData->generation += waiting_t;
            double event = randreal(0, 1);
            CDBG("event = " << event << "\n")
            if (event < totalM / Rate)
                nEvents += migrateEvent(migmap, mRate, totalM);
            else if (event < (totalM + totalC) / Rate)
                nEvents += coalesceEvent(cRate, totalC);
            else if (event < (totalM + totalC + totalR) / Rate)
                nEvents += recombineEvent(rRate, totalR, false, false);
            else if (event < (totalM + totalC + totalR + totalH) / Rate)
                nEvents += recombineEvent(hRate, totalH, true, false);
            else
                nEvents += recombineEvent(dRate, totalD, true, true);
        }
    }
    else
    { // Simulation is running gen-by-gen
        if (timeCheck(1, Rate))
        {
...
bool World::timeCheck(double waiting, double Rate)
{
    ...
    { // haven't reached end of epochs (or we're simulating equilibrium to infinity, inv_age==0)
        if (t >= worldData->epoch_breaks.at(worldData->current_epoch))
        { // waiting time exceeds epoch change
            return true;
        }
        if (Rate == 0)
        {
        ...
        std::cerr << "Error in World::timeCheck(), zero rate will lead to infinite waiting time\n";
    return false;
}
...
void World::updateToNextEpoch()
{
    ...
    {
        std::cerr << "Error in World::updateToNextEpoch(). Current epoch > epoch vector size\n";
        exit(1);
    }
...
void World::demoChange() // we are ignoring but understand this.
{
...
unsigned short World::migrateEvent(vector<vector<double>> &mig_prob, vector<double> &rate, double total)
{
    ...
    double add = 0;
    for (cluster_t::iterator i = cluster.begin(); i != cluster.end(); ++i)
    { // this loop scales the rates to one, and constructs the map
        if (ctxtNcarriers(i->first) != 0)
        { // used to determine in which context migration happened
...
unsigned short World::coalesceEvent(vector<double> &rate, double total)
{
    ...
    {                                                           // carry out the rest of the function.
        ...
        //
        vector<int> carr_idx;
        for (int u = 0; u < worldData->carriers->at(c).size(); ++u)
            carr_idx.push_back(u);
...
unsigned short World::recombineEvent(vector<double> &rate, double total, bool hetero, bool gflux)
{
    ...
    for (cluster_t::iterator i = cluster.begin(); i != cluster.end(); ++i)
    {
        ...
        {
            if (rate[id] != 0)
                whichCarrier[(rate[id] + add) / total] = make_pair(clustID, carrierID);
            add += rate[id];
            ++id;
        }
    }
