...
World::World(shared_ptr<Parameters::ParameterData> p)
{
	...
	// ## is this where we are calculating p and 1-p ? yes this is where its happening.
	worldData->current_epoch = 0;

	if (p->demography.at(0) == 1)
	{
		worldData->epoch_Ncoef = p->demography.at(2); // here we are accesing 3rd index of demography
		if (p->speciation.at(0) == 1)
		{
			worldData->ancesterFreq = p->speciation.at(2); // similarly third index of speciation
	...
	if (worldData->epoch_breaks.size() == 0)
	...
	vector<int> initNCarriers;	  // comment thiese two lines and execute again
	initNCarriers.resize(nClust); // why this is resized to only 4 size?
	shared_ptr<ARGNode> nulle;

	for (int i = 0; i < p->initChr.size(); ++i)
	{ // where did we store the values into this initChr. what is this?
		...
		newNode.reset(new ARGNode(worldData->argNodeVec.size(), chr)); // Make a terminal ARG node
	...
	for (cluster_t::iterator iter = cluster.begin(); iter != cluster.end(); ++iter)
	{
		for (int j = 0; j < worldData->carriers->at((*iter).second).size(); ++j)
		{
			for (int s = 0; s < worldData->snpSites.size(); ++s)
			{
				...
				if (worldData->carriers->at((*iter).second)[j]->carriesSite(site))
					++carrierCounter[s];
			}
		}
	}
...
void World::makecluster(unsigned long nPops)
{
	...
	{ // loop through populations
		for (int j = 0; j < 2; ++j)
		{ // loop through inversion states (I,S)	//loop through alleles in siteA
...
double World::getFreqI(unsigned long pop) // initially it was considered to have more properties(apart from pop and inv). so thats why they are adding to total
{
	double total = 0;
	for (cluster_t::iterator iter = cluster.begin(); iter != cluster.end(); ++iter)
	{
